...
public class ResponseBreaksService {
    ...
    public ResponseService executeService(BasicInputParameters loInput){
        ...
        String lsReturn = "Breaks Response Pooling execute";
        ...
        loBitBean.setLsIndEvento("Ejecución de Servicio de Poleo para Respuesta de Breaks de landmark");
        ...
        String lsWhere = " AND IND_ESTATUS = 'L'"; //enviado a landmak
        ...
        System.out.println("numero de archivos extrados ["+laLis.size()+"]");
        ...
        for(LmkIntXmlFilesRowBean loBean : laList){
            ...
            while(liI < laLis.size() && lbProcess == true){
                ...
                if(loBean.getLsNomFile().contains(laLis.get(liI))){
                    //Es verdadero
                    lbProcess = false;
                    String lsStatus = "C";
                    //Verificar si contiene la cadena de error
                    if(laLis.get(liI).contains("FAIL")){
                        lsStatus = "E";
                    }                    
